...
MAX_FAST_SCREEN_SPONSORS = 180
MAX_FAST_SCREEN_ITEMS_PER_SPONSOR = 3
MAX_FAST_SCREEN_SECONDS = 18.0
FAST_SCREEN_TIMEOUT_SECONDS = 1.15
...
def _fast_screen_query_for_sponsor(source: SponsorEvidenceSource) -> str:
    sponsor_terms = ' OR '.join(f'"{name}"' for name in dict.fromkeys((source.sponsor, *source.aliases)) if name)
    program_terms = ' OR '.join(dict.fromkeys(source.evidence_terms)) or 'ADC OR oncology'
    evidence_terms = ' OR '.join(PROMOTION_TERMS[:18])
    return f'({sponsor_terms}) ({program_terms}) ({evidence_terms})'
...
def _fast_screen_news_items(source: SponsorEvidenceSource) -> list[dict[str, Any]]:
    ...
    """
    query = quote_plus(_fast_screen_query_for_sponsor(source))
...
def _looks_promising_for_promotion(source: SponsorEvidenceSource, raw_item: dict[str, Any]) -> bool:
    ...
    if not _matches_any(text, PROMOTION_TERMS):
        return False
    # Require sponsor/program/lane overlap so generic market articles do not flood
    # the deep evidence set. Sponsor aliases count as overlap only in the fast pass.
    ...
    return bool(alias_terms and (program_terms or context_terms or _matches_any(text, PRESS_RELEASE_TERMS)))
...
def _source_universe(
    ...
    # Rank by strategic evidence utility, not just ticker availability. Mapped
    # tickers remain efficient deep-search handles, but highly relevant discovered
    # biotech/company sponsors should not be crowded out by broad institutional
    # entities.
    def score(src: SponsorEvidenceSource) -> tuple[int, int, str]:
        text = _norm(" ".join((src.sponsor, *src.aliases, *src.evidence_terms)))
        sponsor_penalty = 0
        if any(t in text for t in ("hospital", "university", "institute", "institut", "center", "centre", "group", "network", "foundation", "fundación", "nci", "alliance", "swog", "ecog")):
            sponsor_penalty += 25
        biotech_bonus = -12 if any(t in text for t in ("therapeutics", "biopharma", "biotech", "oncology", "pharmaceutical", "pharma", "medicines", "bioscience")) else 0
        ticker_bonus = -8 if src.tickers else 0
        lane_bonus = -6 if any(t in text for t in ("cdh6", "b7-h4", "b7h4", "vtcn1", "ovarian", "gynecologic", "adc", "antibody-drug")) else 0
        return (src.priority + sponsor_penalty + biotech_bonus + ticker_bonus + lane_bonus, 0 if src.tickers else 1, src.sponsor.lower())

    return sorted(by_name.values(), key=score)
